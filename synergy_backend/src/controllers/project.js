...
export const createProject = async (req, res) => {
  try {
    authenticateToken(req, res, async () => {
      ...
      console.log("Creating project for user:", userEmail);
  ...
  try {
    authenticateToken(req, res, async () => {
      ...
      const updateQuery = `UPDATE project SET ${updateFields.join(
        ", "
      )} WHERE project_id = $${index} RETURNING *;`;
...
export const sendProjectDeadlineReminders = async () => {
  try {
    ...
    for (const project of projects) {
      ...
      if (users.length === 0 || !users[0].email) {
        console.warn(
          `⚠️ No email found for user of project: ${project.project_name}`
        );
        continue;
      }
  ...
  Make the final touches and wrap it up for success! 💪`,
      ...
      console.log(
        `✅ Email sent to ${users[0].email} for project: ${project.project_name}`
      );
    }
  } catch (error) {
